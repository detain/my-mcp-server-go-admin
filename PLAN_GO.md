...
# Implementation Plan: Go MCP Proxy Server - Admin

## Goal

Replace the PHP-based Admin MCP proxy server with a Go implementation that compiles to a single static native binary, supporting Streamable HTTP and STDIO transports, dynamic OpenAPI tool generation, and OAuth 2.1 protected resource metadata.

**Working Directory:** `/home/sites/my-mcp-server-go-admin`
**Reference Implementation:** `/home/sites/my-mcp-server-go-admin/my-mcp-server-php-admin` (PHP version to base off)
...
| Support both Streamable HTTP and STDIO | Match existing PHP implementation features | PHP admin implementation |
...
```bash
cd /home/sites/my-mcp-server-go-admin
go mod init github.com/myadmin/go-mcp-proxy-admin
```

### 1.2 Add Dependencies
```go
// go.mod
module github.com/myadmin/go-mcp-proxy-admin
...
```
/home/sites/my-mcp-server-go-admin/
├── cmd/
│   └── server/
│       └── main.go              # Entry point
├── internal/
...
├── my-mcp-server-php-admin/     # PHP reference (already present)
├── .env.example
├── go.mod
├── go.sum
├── Makefile
...
SESSION_DIR=/tmp/mcp_admin_sessions
CACHE_DIR=/tmp/mcp_admin_cache
...
```bash
go build -o bin/mcp-proxy-admin ./cmd/server
./bin/mcp-proxy-admin --version
```
...
Reference: `/home/sites/my-mcp-server-go-admin/my-mcp-server-php-admin/src/McpServerFactory.php`
...
| `SESSION_DIR` | Session storage directory | `/tmp/mcp_admin_sessions` |
| `CACHE_DIR` | Tool cache directory | `/tmp/mcp_admin_cache` |
...
### 7.1 Create Makefile
```makefile
build: go build -o bin/mcp-proxy-admin ./cmd/server
    ...
    steps:
      ...
      - name: Build
        ...
        run: |
          ...
          go build -ldflags="-s -w" -o bin/mcp-proxy-admin-${{ matrix.goos }}-${{ matrix.goarch }}$ext ./cmd/server
      
      - name: Upload artifacts
        uses: actions/upload-artifact@v4
        with:
          name: mcp-proxy-admin-${{ matrix.goos }}-${{ matrix.goarch }}
          path: bin/*
```
          ...
          for os in linux darwin windows; do
            for arch in amd64 arm64; do
              ...
              GOOS=$os GOARCH=$arch go build -ldflags="-s -w" \
                -o releases/mcp-proxy-admin-$os-$arch$ext ./cmd/server
            done
          done
    ...
    steps:
      ...
      - name: Update GitHub Repository Description and Tags
        run: |
          ...
          gh repo edit --description "Go MCP Proxy Server for MyAdmin Admin API - v$TAG"
...
### 9.3 Create Man Page
Create `man/mcp-proxy-admin.1` man page.
...
- Test against real MyAdmin admin API
...
```bash
npx @modelcontextprotocol/inspector /path/to/mcp-proxy-admin
```
...
5. **Protocol verification:**
   ...
   OPENAPI_SPEC_URL=https://example.com/spec API_BASE_URL=https://example.com/api ./bin/mcp-proxy-admin
...
- Reference PHP implementation at `/home/sites/my-mcp-server-go-admin/my-mcp-server-php-admin`
